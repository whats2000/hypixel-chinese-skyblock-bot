...
class VerifyWeight(CodExtension):

    @commands.command()
    async def verifyweight(self, ctx):
        # check is in the desired channel.
        if ctx.channel.id == get_setting_json('VerifyWeightChannelId'):
            ...
            if get_setting_json('VerifyIdRole') in [y.name.lower() for y in ctx.message.author.roles]:
                ...
                if player_data.api['success']:
                    ...
                    if 'cause' not in player_data.api:
                        player_data.api['cause'] = 'player id is missing'
...
def setup(pybot):
    pybot.add_cog(VerifyWeight(pybot))
